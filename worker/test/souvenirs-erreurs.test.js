/* Tests du classement des erreurs de `js/souvenirs.js`, exécutés sous node avec
   ...
   française qui NOMME la cause — un secret absent, un jeton refusé, un champ
   que Cloudflare ne connaît plus. Une panne de réseau, elle, ne dit rien
   d'utile (« Failed to fetch »). Les deux passaient jusqu'ici par la même
   `ErreurReseau`, indiscernables : l'appelant devait choisir entre montrer un
   charabia de navigateur ou perdre le diagnostic du service. La page
   d'administration a besoin de faire la différence.
...
import { lireConsommation, ErreurReseau, ErreurService } from '../../js/souvenirs.js';
...
test('un 5xx porteur d\'un message garde le statut du service', async () => {
  doublerFetch(corps({ erreur: 'Compteurs indisponibles : les secrets ne sont pas posés.' }, 503));

  const souci = await lireConsommation('mdp').then(() => null, (e) => e);
  ...
  assert.match(souci.message, /secrets ne sont pas posés/);
});
...
test('un refus d\'authentification reste une erreur de service', async () => {
  ...
  const souci = await lireConsommation('mauvais').then(() => null, (e) => e);
...
test('lireConsommation rend le rapport tel que le service le calcule', async () => {
  const rapport = { releveLe: '2026-08-27T09:00:00.000Z', services: [{ nom: 'R2', mesures: [] }] };
  doublerFetch(corps(rapport, 200));

  assert.deepEqual(await lireConsommation('mdp'), rapport);
});
